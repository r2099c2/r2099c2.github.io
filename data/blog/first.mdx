...
如果你只是为了搭建个博客，最简单的方案是直接用一个免费的 tailwinds template，然后直接部署到 github-pages 上。

例如，我正在使用的是 https://github.com/timlrx/tailwind-nextjs-starter-blog 欢迎去给作者 star

你只需要对这个博客进行一些小修改即可使用，例如替换名称、为 Github Actions 配置修改下自动部署规则等。

以下内容是你想手动做一个，或者想了解其中的技术细节在考虑。
