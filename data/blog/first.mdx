...
_PS: 别人搭建好的项目肯定不可能是完全契合你的，所以可以 `fork` 后根据自己的需求进行修改。二次开发注意需要使用 `node 18` 否则启动会有一些问题。_
...
[] 调整导航栏及其中的内容
[] 调整 logo、favicon.ico
[] 注册自己的 newsletter。 PS: 大部分数据配置项在 `siteMetadata.js` 文件中，可以去那里查看。
[] 配置 `Github Actions` 自动部署规则
[] 配置自己的数据监控、SEO 等
...
上面介绍的这个`tailwind nextJs`模板默认就是支持 `vercel` 部署的，我们这里先介绍 `github-pages` 的部署。

官方文档: https://docs.github.com/zh/pages/quickstart

## 2.1 创建一个仓库

你可以直接用上面的模板，然后创建一个仓库，然后将模板 `clone` 到本地，修改后提交。
...
只是简单使用的话，可以放 `html` 或者 `markdown` 文件。
...
_如果只是为了创建博客，以上内容就够了，如果想手动创建或者想了解内部能力，可以继续往下看。_

---

# 3. 手动创建一个 next.js 项目，并增强相关能力

官方文档：https://nextjs.org/docs/getting-started/installation

参照官方文档，创建一个 next.js 项目即可，建议使用最新版本的 `App Router, Typescripts` 等相关技术，配合 `tailwind.css` 使用。
...
至此，基本的能力都已经实现了，你可以在 `contens` 目录下创建 `mdx` 文件，然后在首页和详情页展示。
